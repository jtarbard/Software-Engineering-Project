...
def view_classes_get():
    ...
    activity_names = []
    facility_names = []
    num_activites = len(activity_list)

    for i, activity in enumerate(activity_list):
        activity_capacity = adf.return_activity_capacity_with_activity_type_id(activity.activity_type_id)
        spaces_left = activity_capacity-len(tdf.return_bookings_with_activity_id(activity.activity_id))
        if spaces_left <= 0:
            del activity_list[i]
            continue

        avalibility.append(activity_capacity-len(tdf.return_bookings_with_activity_id(activity.activity_id)))
        ...
        activity_name = adf.return_activity_type_name_with_activity_type_id(activity.activity_type_id)
        if activity_name:
            activity_names.append(activity_name.capitalize())

    return flask.render_template("/activities/classes.html", User=user, activity_list=activity_list,
                                 avalibility=avalibility, num_activites=num_activites,
                                 activity_names=activity_names, facility_names=facility_names)
...
def view_class(activity_id: int):
    ...
    if type(user) is not Customer:
        ...
        membership = udf.return_membership_for_customer_id(account_id)

    duration: datetime.timedelta = activity.end_time - activity.start_time
    total_price = (duration.seconds // 3600 * activity.activity_type.hourly_activity_price)

    return flask.render_template("/activities/class.html", activity=activity, facility=activity.facility,
                                 activity_type=activity.activity_type, total_price=round(total_price, 2), spaces_left=spaces_left,
                                 allow_booking=allow_booking, membership=membership)


@blueprint.route("/activities/view_classes", methods=["POST"])
def view_classes_post():
    ...
    booking_amount: int = int(data_form.get("num_bookings"))
    ...
    if check_out:
        ...
        response = flask.redirect("/activities/classes")
